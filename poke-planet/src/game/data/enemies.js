...
function recordCompleteness(record) {
  return Object.values(METRIC_KEYS).filter((key) => isFiniteMetric(record, key))
    .length;
}
...
export function deduplicateExoplanets(records) {
  ...
  records.forEach((record) => {
    const name =
      typeof record?.pl_name === "string" ? record.pl_name.trim() : "";
    ...
    if (
      !existing ||
      recordCompleteness(record) > recordCompleteness(existing)
    ) {
...
export function classifyEnemyArchetype(percentiles) {
  const ranked = Object.entries(percentiles).sort(
...
function calculateThreatScore(percentiles) {
  const weightedScore = Object.entries(THREAT_WEIGHTS).reduce(
...
export function transformExoplanetToEnemy(record, distributions) {
  ...
  return {
    id: `exoplanet-${String(record.pl_name)
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")}`,
...
export async function createEnemies(
  exoplanetRecords,
  enemyCount,
  { random = Math.random, imageFetcher = fetchImageData } = {},
) {
  ...
  return enemies.sort(
    (first, second) => first.threatScore - second.threatScore,
  );
}
